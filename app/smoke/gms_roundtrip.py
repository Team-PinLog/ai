...
예외 메시지에는 응답 본문 일부가 섞여 오므로 메시지를 그대로 흘리지 않는다. 그 본문은
`-205` 이후 마스킹을 거치지만(failure-recovery.md §2.6) 여기서는 **타입 이름만** 내는
규칙을 유지한다 — 마스킹은 알려진 패턴을 지우는 것이고, 배포 파이프라인 로그는 우리가
읽는 컨테이너 로그보다 보존·열람 범위가 넓다. 이 명령의 stdout·stderr가 거기 남는다.
