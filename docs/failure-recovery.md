...
근거 계약: `static/05_AI_설계.md` §10 실패 복구와 재시도, §6.4 상태 전이, §11 삭제와 경합 방어
...
| 만료 판정과 재스캔 | Spring |
| 최신 Core Context 재조회 후 재전달 | Spring |
| 최대 재시도 소진 시 FAILED 종결 | Spring |
...
- 재시도 시에는 **최신 Core Context를 다시 조회해 전달**해야 하는데(계약 §10.3),
  FastAPI는 `core.*`에 접근할 수 없으므로 오래된 요청 본문밖에 가지고 있지 않습니다.
  FastAPI의 재시도는 구버전 본문 재전송이 됩니다.
- 처리 중 Context가 수정·삭제되었을 수 있고, 그 판단은 State 재조회가 필요합니다.
  재스캔 경로는 이미 그 검사를 포함합니다.
...
- 로그는 `WARN`으로 남기고 `context_id`, `stage`, `context_version`, 원인 분류를 포함합니다.
...
| Context Embedding과 Preset의 Profile 불일치 | 검증 시나리오 9 |
...
- FAILED는 자동으로 되살아나지 않습니다. Context 본문이 수정되어 Spring이 PENDING으로
  초기화할 때만 다시 처리됩니다.
...
| 저장 직전 Version·status 검사 실패 | 결과 폐기 후 정상 종료. 늦은 결과의 정상 경로 |
| State 행 없음 | 정상 종료 |
...
`updated_at` 기준 10분 만료 후 Spring 재스캔이 회수합니다(검증 시나리오 5).
...
4. **재스캔 후보 선택 이후 Context가 삭제된 경우**, 요청은 도착하지만 status가 CANCELLED이므로
   PROCESSING 전환에 실패하고 실행되지 않습니다(검증 시나리오 12).
...
- Keyword가 미완료이거나 실패하면 Keyword만 생략합니다(검증 시나리오 15).
