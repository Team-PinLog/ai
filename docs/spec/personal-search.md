...
- 검색 결과에 유사도 하한 컷오프를 적용하지 않습니다(검색 전용 컷오프 설정 knob은 두지 않음).
  ...
  > 실측(E2E): 무관 질의("자동차 엔진오일 교환 정비소") top-1 유사도 **0.3143**으로 Keyword 하한
  > 0.30을 넘겨, 하한을 검색에 걸었더라도 걸러지지 않았을 것입니다. 관련 질의 top-1 최소 **0.5263** /
  > 무관 top-1 최대 **0.3143**(간격 +0.2120). → 컷오프 미적용 판단을 지지하는 동시에, `0.30`은
  > 검색용 컷오프로는 부적절한 값임을 보입니다(도입한다면 별도 튜닝 필요).
