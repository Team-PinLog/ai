...
- **연결·읽기 타임아웃 분리와 60/90 재산정** (§3.2) — 티켓 제외 범위(A-5). 현재는 단일 total이며,
  ...
  테스트가 지킨다.
- **`integration-tests.md` §4.2의 계층 구분 명문화** — 후속 별건. §4.2는 "HTTP 레벨 목이 아니라
  인터페이스 레벨 Fake"라고만 적어 *파이프라인 시나리오*와 *client 단위* 두 층을 구분하지
  않는다. 중앙과 "층이 다르므로 충돌이 아니다"로 합의했고 근거를 `tests/README.md`에 적었으나,
  명세가 그 구분을 담지 않는 한 같은 의문이 다시 나온다. **`docs/spec/` 수정은 이 티켓의 금지
  범위**이므로 중앙이 별건으로 처리한다.

`docs/WORKLOG.md`는 반영했다. 계약이 금지 범위로 지정한 이유는 당시 `ai#41`이 같은 파일을
건드리고 있었기 때문이며, 그 PR이 CLOSED되고 대체 PR(`ai#42`)이 병합되어 전제가 사라졌음을
중앙이 확인했다.
