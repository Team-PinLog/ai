...
프리셋 27개([2026-07-23-keyword-preset-seed.md](2026-07-23-keyword-preset-seed.md))가 실제 임베딩·판정에서 건전한지, FastAPI·DB 없이 **선행 검증**한다. E(구현) 전에 해야 프리셋 결함 발견 시 재적재·재분류 비용을 피한다. 특히 **테스트 C가 확정하는 판정 프롬프트가 `/context/process`에 그대로 투입**되므로 구현 일부를 미리 끝내는 셈이다.
