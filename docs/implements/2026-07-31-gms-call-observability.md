# AI API 호출·재선점 로그 계측 — 실패율·지연·재선점을 로그로 볼 수 있게 했다
...
| 항목 | 당시 | 원인 |
|---|---|---|
| AI API 요청 성공/실패/지연 | **없음** | 두 클라이언트에 로거가 없었다. 재시도 소진 경고 1건뿐(`retry.py`) |
...
왜 지금이냐면, **AI API 쿼터가 시점·경로에 따라 다르기 때문**이다. 2026-07-29에 "분당 2건"
...
끝나지 못했다는 뜻이고, 원인은 프로세스 종료 아니면 AI API 지연 둘뿐이다. 거짓 양성이
...
성공까지 `INFO`로 남기면 dev 로그가 AI API 호출로 뒤덮여 실패 행을 못 찾는다. 그렇다고
