...
`app/client/_calls.py`(Jira 작업)가 이미 벤더·모델을 로그에 남긴다.
...
1. `ai/.env`(로컬, gitignored, **저장소에 커밋되지 않음**)에서 두 죽은 키를 주석 처리하고
   사유를 적었다:
   ```
   # [Jira 작업] 읽히지 않음 — PINLOG_JUDGE_CHAIN 이 대체(-175). Settings 에 이 alias 필드가 없다. 주석 처리.
   #PINLOG_JUDGE_MODEL=...
   # [Jira 작업] 읽히지 않음 — config.py 에서 이미 제거됨(ai#24, 2026-07-27). 로컬 .env 잔재. 주석 처리.
   #PRESET_CACHE_TTL_SEC=...
   ```
