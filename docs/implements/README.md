...
| I34 | 문서 색인 정합 검사 `tools/check_docs_index.py` — 전수 표의 `T##`·`I##` 중복과, 파일 표 ↔ 파일 시스템의 고아/누락을 `ai-ci / check` 스텝으로 판정. **연속성은 검사하지 않는다**(T9·T10 은 back 레포에 있고 결번은 정상) **두 표의 목록 일치도 검사하지 않는다**(표 이중화가 07-31 사고 3 의 *원인*이라 일치 검사는 문제를 규칙으로 승격시킨다). 셸이 아니라 Python 인 것은 `pytest` 가 같은 함수를 불러 CI 와 로컬이 같은 코드를 쓰기 위함. 새 잡이 아니라 `check` 안의 스텝인 것은 필수 상태 검사가 두 이름뿐이라 잡을 늘리면 실패해도 병합을 못 막기 때문. 메시지는 **다음 빈 번호와 붙여 넣을 표 행까지** 준다. 착수 시점 `dev` 위반 3건(고아) + 표 렌더링 6곳 정리. 조사 결과 **표 이중화는 축소 불가** — 트러블슈팅 전수 표는 문서 링크가 0종이고 구현 전수 표는 파일 표 24개 중 17개만 덮으며 13행은 문서가 아닌 산출이다 | [색인 검사 리포트](2026-07-31-docs-index-check.md), [T61·T62](../troubleshooting/2026-07-31-docs-index-check.md), [tools/check_docs_index.py](../../tools/check_docs_index.py) |
