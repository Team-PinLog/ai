...
**원인** `merge=union` 은 충돌을 「양쪽 다 채택」으로 푼다. 새 행을 각자 추가한 경우에는 그것이 정확히 원하는 동작이다. 그런데 **한쪽이 기존 행을 고치면** git 은 그것을 「지운 것 + 새로 넣은 것」으로 보고, 지운 쪽을 되살린다.
...
같은 날 `T43` 번호 충돌(T48)과 원인이 다르다 — 그쪽은 두 브랜치가 **같은 번호를 각자 잡은 것**이고, 이쪽은 **한 브랜치가 자기 행을 갱신한 것**이다. 전자는 union 이 못 막고 후자는 union 이 만든다.
