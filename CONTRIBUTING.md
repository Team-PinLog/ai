...
**「릴리스 시점」을 정해 두지 않아 2026-07-31 에 `main` 이 `dev` 보다 16커밋
뒤처졌다.** dev 클러스터가 쓰던 이미지는 그보다 앞선 `#42`(07-29) 였고, 그
사이에 병합된 판정 LLM 벤더 폴백·재시도 개선·계측이 **어디에도 배포되지
않은 상태**였다. 아무도 규약을 어기지 않았고, 규약이 시점을 말하지 않았다.

다음 중 하나라도 해당하면 릴리스 PR 을 연다.

```
배포 경로에 영향을 주는 변경이 dev 에 들어갔을 때   앱 코드·설정·의존성
그 자체로 시연·검증에 필요한 기능이 병합됐을 때
main 이 dev 보다 10커밋 이상 뒤처졌을 때
```

릴리스 PR 은 `release/{YYYY-MM-DD}` 브랜치로 열고 base 는 `main` 이다. 본문에
**무엇이 처음 배포되는지**를 나열한다 — 목록이 곧 무엇이 여태 배포되지 않았는지의
기록이 된다.

`main` 에 직접 들어간 변경(주로 `infra` 가 내는 워크플로 수정)이 있으면 릴리스
브랜치에서 `main` 을 먼저 병합해 충돌을 해소한 뒤 PR 을 연다. **그 해소를 PR 화면에
맡기지 않는다** — 어느 쪽을 채택했는지가 커밋 메시지에 남아야 한다.

병합 후 이미지가 게시되면 **`infra` 에 `values.yaml` 의 `image.tag` 갱신을
요청한다.** 게시와 반영은 다른 일이고, 후자는 AI 파트 권한이 아니다.
