...
- 주차·화장실·직원 응대·가격 같은 부대시설이나 서비스 이야기는 장소의 Keyword가 아닙니다. 장소에서 무엇을 했는지·누구와·어떤 분위기였는지만 고르세요.
- confidence는 근거의 확실함을 0~1로 나타냅니다. 애매하면 낮게 줍니다.
```

> 부대시설/서비스 제외 규칙은 테스트 C-1에서 추가했습니다. "주차가 넓어서" 같은
> 입력이 SPACIOUS로 오탐되는 것을 판정 단계에서 막습니다(후보 하한을 올리면 약한
> 진짜 매칭까지 잃으므로, 하한 0.30을 유지하고 이 규칙으로 정밀도를 보완).
